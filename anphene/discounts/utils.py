...
def get_product_discount_on_sale(product, product_collections, discount):
...
def get_product_discounts(product, collections, discounts):
...
def calculate_discounted_price(product, price, collections, discounts):
    """Return minimum product's price of all prices with discounts applied."""
    if discounts:
        discount_prices = list(get_product_discounts(product, collections, discounts))
        if discount_prices:
...
def get_products_voucher_discount(voucher, prices):
    ...
    if voucher.apply_once_per_order:
        return voucher.get_discount(min(prices))
    discounts = (voucher.get_discount(price) for price in prices)
    total_amount = sum(discounts)
    return total_amount
...
def _fetch_categories(sale_pks):
    ...
    categories = Sale.categories.through.objects.filter(sale_id__in=sale_pks).values_list(
        "sale_id", "category_id"
    )
...
def _fetch_collections(sale_pks):
    collections = Sale.collections.through.objects.filter(sale_id__in=sale_pks).values_list(
